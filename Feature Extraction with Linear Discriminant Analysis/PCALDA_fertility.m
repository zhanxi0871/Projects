close all
% load data
fertilitydata = csvread('fertilitydata.csv')
species = importdata('fertilityclass.csv');
...
for k = 1:n0
     if isequal(species(k),ClassLabel(1))
        ...
        isequal(species(k),ClassLabel(2))
        species2(k) = 2;
    end
end
...
irisdata_train = fertilitydata(idx_train,:);
...
irisdata_test = fertilitydata(idx_test,:);
